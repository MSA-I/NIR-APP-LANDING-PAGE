...
//
// The palette is two colours (see the tokens block in src/styles.css) and a
// theme is nothing more than which of them is underneath. All of the switching
// happens in CSS: `:root[data-theme="light"]` redeclares the same token names
// with the two colours' jobs swapped, and Tailwind compiles every utility to
// `var(--color-…)`, so one attribute on <html> moves the authored stylesheet
// and the utility classes together.
//
...
const BROWSER_CHROME: Record<Theme, string> = { dark: '#020202', light: '#b2d5e5' }
