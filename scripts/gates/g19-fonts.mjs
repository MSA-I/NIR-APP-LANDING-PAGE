  ...
  await withPage(
    async (page) => {
      const measured = await page.evaluate((word) => {
        ...
        return {
          displayLoaded: document.fonts.check(`600 42px "LT Superior"`, word),
          monoLoaded: document.fonts.check('500 18px "Hasubi Mono"', word),
          displayWidth: width('"LT Superior"'),
          missingWidth: width('"No Such InPlace Face"'),
          monoWidth: width('"Hasubi Mono"', 500),
          monoMissingWidth: width('"No Such InPlace Mono"', 500),
          h1: family('h1'),
      ...
      c.ok(Math.abs(measured.monoWidth - measured.monoMissingWidth) > 1, `${name}: Hasubi Mono measures like fallback`)
