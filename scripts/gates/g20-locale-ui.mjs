...
try {
  for (const test of cases) {
    ...
    try {
      ...
      await page.waitForFunction(() => document.activeElement?.classList.contains('language-switcher__option'))
      const firstOption = menu.locator('a').first()
      c.ok(
        await firstOption.evaluate((element) => element === document.activeElement),
        `${test.name}: ArrowDown opened the disclosure but did not focus its first link`,
      )
      ...
      await page.waitForFunction(() => {
        ...
        return selected && selected === document.activeElement
      })
      ...
      const focusedTab = await page.evaluate(() => document.activeElement?.getAttribute('id'))
      c.ok(focusedTab === after, `${test.name}: selection moved to ${after}, but focus stayed on ${focusedTab}`)
