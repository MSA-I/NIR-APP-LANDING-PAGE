// G7: text contrast clears WCAG AA on every act, at both grounds, in both themes.
//
...
/** Source-over: what a partly transparent ink actually looks like on a ground. */
const over = (fg, bg) =>
  fg[3] >= 0.999
    ? [fg[0], fg[1], fg[2]]
...
await withPage(async (page) => {
  ...
  // TWO THEMES, TWO WALKS.
  // The palette is two colours with their jobs swapped between the themes (see
  // the tokens block in src/styles.css), so a clean sweep on the dark page is
  // no evidence at all about the light one: every pair that reads on black has
  // a mirror on candy blue, and a mirror can be wrong on its own.
  //
  ...
  // rather than being photographed in the wrong theme.
  for (const theme of ['dark', 'light']) {
    ...
    for (const s of stops) {
      ...
      for (const r of rows) {
        ...
        // Ink is composited over that ground before it is graded.
        //
        // This stylesheet writes a great deal of its secondary type as
        // `color-mix(in srgb, <token> 58%, transparent)`, and grading that at
        // its full strength reports a contrast the reader never gets. It also
        // fixes the opposite error: `.footer-wordmark` declares
        // `color: transparent` and paints through `background-clip: text`, and
        // reading that as opaque BLACK graded a ghost at 13.54:1 on the light
        // page, which is a measurement defect reported as a design defect.
    ...
    // A run that declares no ink cannot fail the floor by arithmetic, so it is
    // failed by name instead. `.footer-wordmark` is allowed to do this because
    // it is marked decorative and is graded by the ceiling below; ordinary copy
    // that paints nothing is a bug the ratio would never catch.
  ...
  // Back to the page's home state before the control below, which is written
  // against the dark page's cream plate.
  await page.evaluate(() => {
