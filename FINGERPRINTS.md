...
| 3 | `inplace-folio` | Chaptered editorial (a title page and six chapters; one scrub chapter, everything else ordinary document flow) | A folio, not a bar: wordmark, the **live chapter name** updating as chapters pass, and one ask. It changes ground with the page — dark over Onyx, cream over the wheat plate. No links, no menu. | A **title page**. Type on the Onyx ground, no media above the fold, one ask, and a printed index of all six chapters with their subtitles. The film does not start until chapter 01. | A title page and six chapters, 13.71vh: title page · **scrub 5.9 (the one act)** · cream plate, chain of five, board spread · two-column refusal list · plan table · FAQ on a second cream plate · colophon. One device on the whole page; every other section is ordinary document flow. | A **colophon**: one heading, one line, one button, and a real footer under it. The source list that used to sit beside it is gone. No spotlight, no magnet, no input, no arrival. | **המסירה** — the film does not cut, it hands over. Chapter 01 dissolves out of the world and into the product's own control centre, filling the frame head-on, and every pixel after that moment on the page is a screenshot of the running system. The handover is the argument: the reader watches until the exact second there is something real to look at. Replaced the citation apparatus, which the owner removed on 26.08.2026 as confusing rather than reassuring. |
...
- **The film as a chapter rather than a hero**: reusing an earlier build's
  ...
  as a look.
- **The apparatus**: a page that sources its own figures, with superscript
  markers, a floating footnote and a source list. Built, shipped, then removed
  at the owner's instruction: he could not tell what the markers were for, and
  neither could a visitor. **Spent as an idea, and the lesson is sharper than
  the idea**: a device that has to be explained is not reassurance, it is a
  second thing to learn. The sources still live in `i18n/he.js`, where a
  maintainer needs them and a visitor does not.
- **Length band**: 13.71vh. Rows now sit at
  12.3, 14.15 and 16.0, and prior scrollcraft builds clustered at 13.6-13.8, so
  this row now sits inside that cluster, which is the one place it should not
  be. Content decided the length four times running. Free
  space left: below 12, and above 16.5. Length here was decided by content
  three times in a row, which is exactly the thing the next build should not
  repeat.
